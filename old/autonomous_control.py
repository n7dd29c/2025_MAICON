...
class AutonomousController:
    ...
    def get_control_command(self, 
        ...
        # 2순위: ArUco 마커 명령 (화재 건물 마커)
        if aruco_command is not None and aruco_command.get('action') != 'go_straight':
            action = aruco_command.get('action', 'go_straight')
            # 화재 건물 마커인 경우 설정된 각도 사용, 아니면 기본 각도 사용
            turn_angle = aruco_command.get('turn_angle')
            if turn_angle is None:
                from config import ARUCO_TURN_ANGLE
                turn_angle = ARUCO_TURN_ANGLE
            from config import ARUCO_TURN_SPEED
            
            if action == 'turn_right':
                steering_angle = turn_angle  # 우회전
                safety_message = f"화재 건물: 우회전 ({turn_angle}도)"
            elif action == 'turn_left':
                steering_angle = -turn_angle  # 좌회전
                safety_message = f"화재 건물: 좌회전 ({turn_angle}도)"
            elif action == 'stop':
                steering_angle = 0.0
