...
class AutonomousDriving:
    ...
    def __init__(self,
        ...
        # 화재 섹터 정보 읽기 초기화
        from config import FIRE_SECTOR_REPORT_DIR, FIRE_SECTOR_JSON_FILENAME, MISSION_CODE
        self.fire_sector_reader = FireSectorReader(
            report_dir=FIRE_SECTOR_REPORT_DIR,
            json_filename=FIRE_SECTOR_JSON_FILENAME
        )
        # 초기 화재 섹터 정보 읽기 시도 (파일이 없으면 대기하지 않음)
        self.fire_sector_reader.read_fire_sectors()
        
        # 화재 건물 마커 캡쳐 상태
        self.fire_building_capture_state = None  # None, 'turning', 'capturing', 'returning'
        self.fire_building_capture_start_time = None
        self.fire_building_last_captured_sector = None  # 중복 캡쳐 방지
        self.fire_building_turn_action = None  # 회전 방향 저장 ('turn_left' or 'turn_right')
    ...
    def process_frame(self, frame: np.ndarray) -> tuple:
        ...
        if USE_ARUCO and self.aruco_detector is not None:
            ...
            if aruco_command is not None:
                ...
                if marker_sector is None:
                    marker_sector = self.current_sector
                
                # 화재 건물 섹터인지 확인
                is_fire_building = self.fire_sector_reader.is_fire_building_sector(marker_sector)
                
                # 대시보드로 ArUco 마커 감지 정보 전송 (통신 보고)
                if USE_DASHBOARD and self.dashboard is not None:
                    # 마커에 섹터가 설정되어 있으면 로봇 섹터 업데이트
                    if marker_sector != self.current_sector:
                ...
                # 화재 건물 마커인 경우에만 행동 제어
                if is_fire_building:
                    # 화재 건물 마커 감지 시 45도 회전 → 캡쳐 → 원상복구
                    if (self.fire_building_capture_state is None and 
                        marker_sector != self.fire_building_last_captured_sector):
                        # 1단계: 회전 시작
                        self.fire_building_capture_state = 'turning'
                        self.fire_building_capture_start_time = time.time()
                        self.fire_building_last_captured_sector = marker_sector
                        self.fire_building_turn_action = action  # 회전 방향 저장
                        
                        from config import FIRE_BUILDING_TURN_ANGLE, FIRE_BUILDING_TURN_DURATION
                        # ArUco 명령 저장 (45도 회전)
                        aruco_command_with_angle = aruco_command.copy()
                        aruco_command_with_angle['turn_angle'] = FIRE_BUILDING_TURN_ANGLE
                        self.current_aruco_command = aruco_command_with_angle
                        self.aruco_command_start_time = time.time()
                        self.aruco_command_duration = FIRE_BUILDING_TURN_DURATION
                        print(f"[화재 건물] 마커 감지 (ID={marker_id}, Sector={marker_sector}) - {action} 방향으로 {FIRE_BUILDING_TURN_ANGLE}도 회전 시작")
                    elif self.fire_building_capture_state == 'turning':
                        # 회전 중 → 회전 완료 후 캡쳐
                        from config import FIRE_BUILDING_TURN_DURATION, FIRE_BUILDING_CAPTURE_DELAY
                        elapsed = time.time() - self.fire_building_capture_start_time
                        if elapsed >= FIRE_BUILDING_TURN_DURATION + FIRE_BUILDING_CAPTURE_DELAY:
                            # 2단계: 캡쳐
                            self.fire_building_capture_state = 'capturing'
                            self._capture_fire_building(frame, marker_sector, marker_id)
                            # 3단계: 원상복구 시작 (반대 방향으로 회전)
                            self.fire_building_capture_state = 'returning'
                            self.fire_building_capture_start_time = time.time()
                            # 반대 방향으로 회전 명령 생성
                            return_action = 'turn_left' if self.fire_building_turn_action == 'turn_right' else 'turn_right'
                            return_command = {
                                'action': return_action,
                                'marker_id': marker_id,
                                'turn_angle': FIRE_BUILDING_TURN_ANGLE
                            }
                            self.current_aruco_command = return_command
                            self.aruco_command_start_time = time.time()
                            from config import FIRE_BUILDING_RETURN_DURATION
                            self.aruco_command_duration = FIRE_BUILDING_RETURN_DURATION
                            print(f"[화재 건물] 캡쳐 완료 - 원상복구 시작 ({return_action} 방향으로 {FIRE_BUILDING_TURN_ANGLE}도 회전)")
                    elif self.fire_building_capture_state == 'returning':
                        # 원상복구 중 → 완료 후 상태 초기화
                        from config import FIRE_BUILDING_RETURN_DURATION
                        elapsed = time.time() - self.fire_building_capture_start_time
                        if elapsed >= FIRE_BUILDING_RETURN_DURATION:
                            # 원상복구 완료
                            self.fire_building_capture_state = None
                            self.current_aruco_command = None
                            self.aruco_command_start_time = None
                            self.fire_building_turn_action = None
                            print(f"[화재 건물] 원상복구 완료 - 정상 주행 재개")
                else:
                    # 화재 건물이 아니면 ArUco 제어 안 함 (통신 보고만)
                    self.current_aruco_command = None
                    print(f"ArUco 마커 감지 (통신 보고만, 화재 건물 아님): ID={marker_id}, Sector={marker_sector}, Action={action}")
            else:
                # ArUco 명령이 없으면 이전 명령 지속 시간 확인
                if self.fire_building_capture_state == 'turning':
                    # 회전 중인데 마커가 사라진 경우
                    from config import FIRE_BUILDING_TURN_DURATION, FIRE_BUILDING_CAPTURE_DELAY
                    if self.fire_building_capture_start_time is not None:
                        elapsed = time.time() - self.fire_building_capture_start_time
                        if elapsed >= FIRE_BUILDING_TURN_DURATION + FIRE_BUILDING_CAPTURE_DELAY:
                            # 회전 완료 후 캡쳐 (마커가 사라져도)
                            self.fire_building_capture_state = 'capturing'
                            # 현재 프레임으로 캡쳐
                            if self.fire_building_last_captured_sector:
                                self._capture_fire_building(frame, self.fire_building_last_captured_sector, None)
                            # 원상복구 시작
                            self.fire_building_capture_state = 'returning'
                            self.fire_building_capture_start_time = time.time()
                            # 반대 방향으로 회전 명령 생성
                            if self.fire_building_turn_action:
                                return_action = 'turn_left' if self.fire_building_turn_action == 'turn_right' else 'turn_right'
                                return_command = {
                                    'action': return_action,
                                    'marker_id': None,
                    ...
                    if self.fire_building_capture_start_time is not None:
                        ...
                        if elapsed >= FIRE_BUILDING_RETURN_DURATION:
                            ...
                            self.fire_building_turn_action = None
                            print(f"[화재 건물] 원상복구 완료 - 정상 주행 재개")
                elif self.aruco_command_start_time is not None:
        ...
        # 제어 명령 생성 (우선순위: 포트홀 회피 > 화재 건물 ArUco 명령 > 차선 추종)
        # 화재 건물 마커인 경우에만 ArUco 제어 사용
        aruco_command_for_control = None
        if self.current_aruco_command is not None:
            # 화재 건물 마커인 경우에만 제어 명령 전달
            aruco_command_for_control = self.current_aruco_command
    ...
    def _capture_fire_building(self, frame: np.ndarray, sector: str, marker_id: Optional[int]):
        """
        화재 건물 사진 캡쳐 및 대시보드 전송
        ...
        Args:
            ...
            marker_id: 마커 ID (선택적)
        """
        from config import FIRE_BUILDING_CAPTURE_DIR
        from pathlib import Path
        from datetime import datetime
        
        # 캡쳐 디렉터리 생성
        capture_dir = Path(FIRE_BUILDING_CAPTURE_DIR)
        capture_dir.mkdir(exist_ok=True)
        
        # 파일명 생성 (타임스탬프 + 섹터)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"fire_{sector}_{timestamp}.jpg"
        if marker_id is not None:
            filename = f"fire_{sector}_marker{marker_id}_{timestamp}.jpg"
        ...
        try:
            ...
            print(f"[화재 건물] 사진 캡쳐 완료: {filepath}")
            
            # 대시보드로 전송
            if USE_DASHBOARD and self.dashboard is not None:
                success = self.dashboard.send_dashboard_image(str(filepath))
                if success:
                    print(f"[화재 건물] 대시보드 전송 완료: {filename}")
                else:
                    print(f"[화재 건물] 대시보드 전송 실패: {filename}")
        except Exception as e:
            print(f"[화재 건물] 캡쳐 오류: {e}")
